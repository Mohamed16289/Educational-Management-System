...
def read_data_from_file(file_name):
    if os.path.exists(file_name):
        ...
        return pd.DataFrame(columns=['user_id', 'username', 'password', 'full_name', 'email', 'user_type'])
...
def main():
    ...
    courses = []
    file_name = "Data.txt"
    user_data = read_data_from_file(file_name)

   
    for _, row in user_data.iterrows():
        if row['user_type'] == 'Doctor':
            user = Doctor(row['user_id'], row['username'], row['password'], row['full_name'], row['email'])
        elif row['user_type'] == 'Student':
            user = Student(row['user_id'], row['username'], row['password'], row['full_name'], row['email'])
        elif row['user_type'] == 'TA':
            user = TA(row['user_id'], row['username'], row['password'], row['full_name'], row['email'])
        users.append(user)
    ...
    while True:
        ...
        if choice == '1':
            ...
            new_user_data = pd.DataFrame({
                ...
                'full_name': [full_name],
                'email': [email],
                'user_type': ['Doctor' if user_type == '1' else 'Student' if user_type == '2' else 'TA']
            })

            write_data_to_file(file_name, new_user_data)
